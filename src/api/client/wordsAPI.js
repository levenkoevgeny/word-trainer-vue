...
export const wordsAPI = {
  async getItemsList(token, query = {dictionary: "", word_rus: "", word_eng: "", limit: ""}) {
    let {dictionary, word_rus, word_eng, limit} = query
    return axios.get(
      `${process.env.VUE_APP_BACKEND_PROTOCOL}://${process.env.VUE_APP_BACKEND_HOST}:${process.env.VUE_APP_BACKEND_PORT}/api/${base_url}/?word_rus__icontains=${word_rus}&word_eng__icontains=${word_eng}&dictionary=${dictionary}&limit=${limit}`,
      authHeaders(token),
    )
  },
